zokou({
  ...
  execute: async (message, args) => {
    try {
      ...
      if (message.client && message.client.sendMessage) {
        await message.client.sendMessage(message.chat, { text: replyText, quoted: fakeMsg });
        console.log(`Fake reply sent to ${num}: ${replyText}`);
      } else {
        console.log("Error: message.client.sendMessage is not a function or message.client is undefined");
        return message.reply("❌ *Error:* Sending message failed due to internal error.");
      }
    } catch (e) {
      console.error(e);
