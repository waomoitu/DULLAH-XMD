...
zokou({
  ...
  execute: async (message, args) => {
    try {
      ...
      let replyText = input[0]; // Message to send
      let msgText = input[1]; // Fake reply content
      ...
      // Ensure msgText is not empty
      if (!msgText) {
        return message.reply("❌ *Error:* Reply text cannot be empty.");
      }
