...
authentification();
    async function main() {
        ...
        const sockOptions = {
            ...
            browser: ['dullah-Md', "safari", "1.0.0"],
...
// Other functions (auto-react, anti-delete, etc.) as needed
  ...
  if (conf.ANTICALL === 'yes') {
    ...
    await zk.sendMessage(callerId, {
      text: "Hello😊,am Dullah-Md a personal assistant,please try again later"
    });
  }
});
    ...
    async  function activateCrons() {
        ...
        zk.ev.on("connection.update", async (con) => {
            ...
            if (connection === "connecting") {
                console.log("ℹ️ dullah is connecting...");
            }
            else if (connection === 'open') {
                console.log("✅ dullar Connected to WhatsApp! ☺️");
                console.log("--");
                await (0, baileys_1.delay)(200);
                console.log("------");
                await (0, baileys_1.delay)(300);
                console.log("------------------/-----");
                console.log("dullar is Online 🕸\n\n");
                //chargement des commandes 
                console.log("Loading dullar Commands ...\n");
                fs.readdirSync(__dirname + "/dullah").forEach((fichier) => {
                    if (path.extname(fichier).toLowerCase() == (".js")) {
                        try {
                            require(__dirname + "/dullah/" + fichier);
